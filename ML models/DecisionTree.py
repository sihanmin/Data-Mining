...
def compute_info_gain(data, att_name, y_name):
    ...
    zipdata = data[[att_name, y_name]]
    
    for val,count in zip(vals, counts):
        subdata = zipdata[zipdata[att_name] == val]
        conditional_entropy += count / total_counts * compute_entropy(subdata[y_name])

    info_gain = total_info - conditional_entropy
    #print(att_name, conditional_entropy, info_gain)
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    #print(att_name, att_info,gain_ratio, ig)
    return gain_ratio
