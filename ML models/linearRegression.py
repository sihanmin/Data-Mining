...
def getBeta(train_x, train_y):   
    ...
    x_transpose = np.transpose(train_x) # X^T
    inverse = np.linalg.inv(x_transpose.dot(train_x)) # (X^T*X)^ -1
    beta = inverse.dot(x_transpose.dot(train_y))
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
       deriv = np.zeros(p)
       for i in range(n):
           deriv += train_x[i].dot(train_x[i].transpose().dot(beta) - train_y[i])
       deriv = deriv / n
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            idx = indices[i]
            beta += (lr * (train_y[idx] - train_x[idx].transpose().dot(beta)) * train_x[idx])
    return beta
