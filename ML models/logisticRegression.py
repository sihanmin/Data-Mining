...
def compute_avglogL(X,y,beta):
    ...
    logL = 0
    for i in range(n):
        xTB = X[i].transpose().dot(beta)
        logL += (y[i] * xTB - np.log(1 + np.exp(xTB)))
    avglogL = logL / n
    return avglogL
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        '''for j in range(p):
            for i in range(n):
                xTB = train_x[i].transpose().dot(beta)
                deriv[j] += train_x[i][j] * (train_y[i] - sigmoid(xTB))'''
        for i in range(n):
            xTB = train_x[i].transpose().dot(beta)
            diff = train_y[i] - sigmoid(xTB)
            deriv += train_x[i] * diff
...
# lr (learning rate) is a scalar
# lamda is a scalar
# function returns value of beta calculated using (0) regularized batch gradient descent
def getBeta_BatchGradient_Regularized(train_x, train_y, lr, num_iter, verbose, lamda):
    beta = np.random.rand(train_x.shape[1])
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        for i in range(n):
            ...
            piB = sigmoid(xTB)
            diff = train_y[i] - piB
            deriv1st += train_x[i] * diff
            for j in range(p):
                for k in range(p):
                    deriv2nd[j][k] -= train_x[i][j] * train_x[i][k] * piB * (1-piB)
        
        beta = beta - np.linalg.inv(deriv2nd).dot(deriv1st)
...
class LogisticRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            beta = getBeta_BatchGradient_Regularized(newTrain_x, self.train_y.values, self.lr, self.num_iter, self.verbose, self.lamda)
            print('Beta: ', beta)
        else:
...
if __name__ == '__main__':
    # Change 1st paramter to 0 for batch gradient, 1 for newton's method, 2 for regularized batch gradient
